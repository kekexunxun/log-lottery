<template>
  <div class="home-background w-screen h-screen overflow-hidden" v-if="configStore.theme.background.url">
    <img :src="imgUrl" class="w-full h-full object-cover" alt="" />
  </div>
  <div v-else class="w-screen h-screen overflow-hidden" ref="starRef"></div>
</template>
...
onMounted(() => {
  getImageStoreItem(configStore.theme.background).then((image) => {
    imgUrl.value = image
  })
  addSparticles(starRef.value, width.value, height.value)
  listenWindowSize()
